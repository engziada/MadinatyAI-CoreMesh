...
describe('TokensController', () => {
  const tokensService = {
    ...
    allocate: jest.fn().mockResolvedValue({ userId: 'u-1', businessTokens: 100, individualTokens: 0 }),
    getWallet: jest.fn().mockResolvedValue({ userId: 'u-1', businessTokens: 100, individualTokens: 20 }),
    listActivityPricing: jest.fn().mockResolvedValue([{ activityType: 'kitchen_rental', cost: 50 }]),
  ...
  it('spends tokens via POST /spend', async () => {
    const result = await controller.spend({ userId: 'u-1', activityType: 'kitchen_rental', tokenType: 'business' });
    expect(tokensService.spend).toHaveBeenCalledWith('u-1', 'kitchen_rental', 'business', undefined);
    expect(result.businessTokens).toBe(40);
  });

  it('allocates tokens via POST /allocate', async () => {
    await controller.allocate({ userId: 'u-1', activityType: 'souk_ad', tokenType: 'business', amount: 30 });
  ...
  it('sets pricing via POST /pricing', async () => {
    const result = await controller.setPricing({ activityType: 'kitchen_rental', cost: 50, description: 'Kitchen rental' });
    expect(tokensService.setActivityPricing).toHaveBeenCalledWith('kitchen_rental', 50, 'Kitchen rental', undefined);
